...
public class TestSummaryCreatorTask extends Task {
	...
	/**
	 * Returns the list of input files ({@link java.io.File} objects) to
	 * process. Note that this does not check that the file is a valid and
	 * useful XML file.
	 * 
	 ...
	 *            The XML document
	 * @param xpath
	 *            The xpath selecting the node whose attribute we want
	 * @param attrName
	 ...
	 * 
	 * @param summaryDoc
	 *            The document object to add data to
	 * @param inputFile
	 *            The input file
	 */
	private void processInputFile(Document summaryDoc, File inputFile)
			throws IOException {
		try {
			// First we check whether it is an XML file
			SAXReader reader = new SAXReader();
			Document testDoc = reader.read(new InputSource(new FileReader(
					inputFile)));
			...
			Element generalElement = (Element) summaryDoc
			...
			if (generalElement == null) {
				generalElement = summaryDoc.getRootElement().addElement(
						"general");

				// General run info (we only need this once)
				generalElement.addAttribute("ddlUtilsVersion", _version);
				generalElement.addAttribute(
						"date",
						getAttrValue(testDoc, "//property[@name='TODAY']",
								"value"));
				generalElement.addAttribute(
						"lang",
						getAttrValue(testDoc, "//property[@name='env.LANG']",
								"value"));
				generalElement
						.addAttribute(
								"jre",
								getAttrValue(
										...
										+ getAttrValue(
												testDoc,
												"//property[@name='java.vendor']",
												"value") + ")");
				generalElement
						.addAttribute(
								"os",
								getAttrValue(testDoc,
										"//property[@name='os.name']", "value")
										+ " "
										+ getAttrValue(
												testDoc,
										...
										+ ", arch "
										+ getAttrValue(testDoc,
												"//property[@name='os.arch']",
												"value"));
				addTargetDatabaseInfo(
						generalElement,
						getAttrValue(testDoc,
								"//property[@name='jdbc.properties.file']",
								"value"));
			} else {
				totalNumTests = Integer.parseInt(generalElement
						.attributeValue("tests"));
				totalNumErrors = Integer.parseInt(generalElement
						.attributeValue("errors"));
				totalNumFailures = Integer.parseInt(generalElement
						.attributeValue("failures"));
			}

			int numTests = Integer.parseInt(getAttrValue(testDoc, "/testsuite",
					"tests"));
			...
			generalElement.addAttribute("failures",
					String.valueOf(totalNumFailures));

			if ((numErrors > 0) || (numFailures > 0)) {
				Element testSuiteNode = (Element) testDoc
						.selectSingleNode("/testsuite");
				String testSuiteName = testSuiteNode.attributeValue("name");

				// since tests have failed, we add it to the summary
				for (Iterator it = testSuiteNode.selectNodes(
						"testcase[failure or error]").iterator(); it.hasNext();) {
					Element failedTestElement = (Element) it.next();
					Element newTestElement = summaryDoc.getRootElement()
							.addElement("failedTest");

					// Test setup failure, so the test was not actually run ?
					if (!failedTestElement.attributeValue("classname")
							.startsWith("junit.framework.TestSuite")) {
						newTestElement.addAttribute(
								"name",
	 ...
	 * 
	 * @param element
	 *            The element to add the relevant database properties to
	 * @param jdbcPropertiesFile
	 *            The path of the properties file
	 */
	protected void addTargetDatabaseInfo(Element element,
			String jdbcPropertiesFile) throws IOException, BuildException {
		...
		try {
			String dataSourceClass = props.getProperty(
					TestAgainstLiveDatabaseBase.DATASOURCE_PROPERTY_PREFIX
							+ "class", BasicDataSource.class.getName());
			...
			for (Iterator it = props.entrySet().iterator(); it.hasNext();) {
				...
				if (propName
						.startsWith(TestAgainstLiveDatabaseBase.DATASOURCE_PROPERTY_PREFIX)
						&& !propName
								.equals(TestAgainstLiveDatabaseBase.DATASOURCE_PROPERTY_PREFIX
										+ "class")) {
					BeanUtils
							.setProperty(
									dataSource,
									propName.substring(TestAgainstLiveDatabaseBase.DATASOURCE_PROPERTY_PREFIX
											.length()), entry.getValue());
				}
			}

			String platformName = props
					.getProperty(TestAgainstLiveDatabaseBase.DDLUTILS_PLATFORM_PROPERTY);
			...
			try {
				element.addAttribute("dbProductName",
						metaData.getDatabaseProductName());
			} catch (Throwable ex) {
				// we ignore it
			}
			try {
				element.addAttribute("dbProductVersion",
			...
			try {
				...
				element.addAttribute("dbVersion", databaseMajorVersion + "."
			...
			try {
				element.addAttribute("driverVersion",
			...
			try {
				...
				element.addAttribute("jdbcVersion", jdbcMajorVersion + "."
						+ jdbcMinorVersion);
			} catch (Throwable ex) {
				// we ignore it
			}
	...
	private Properties readProperties(String jdbcPropertiesFile) {
		...
		try {
			propStream = TestSummaryCreatorTask.class
			...
			if (propStream == null) {
				...
				if (propFile.exists() && propFile.isFile()
					...
					throw new BuildException(
							"Cannot load test jdbc properties from file "
									+ jdbcPropertiesFile);
				}
			}
			...
			throw new BuildException(
					"Cannot load test jdbc properties from file "
							+ jdbcPropertiesFile, ex);
		} finally {
			if (propStream != null) {
				try {
					propStream.close();
				} catch (IOException ex) {
					log("Could not close the stream used to read the test jdbc properties",
							Project.MSG_WARN);
				}
			}
		}
	...
	public void execute() throws BuildException {
		try {
			...
			throw new BuildException(
					"Error while processing the test results: "
