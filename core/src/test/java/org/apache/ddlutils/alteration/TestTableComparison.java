...
public class TestTableComparison extends TestComparisonBase {
	...
	public void testAddColumn() {
		...
		assertColumn("Col1", Types.DOUBLE, null, null, false, false, false,
				change.getNewColumn());
		assertEquals("ColPK", change.getPreviousColumn());
	 ...
	 * (which usually returns sizes for every column) and a model from XML. The
	 * model comparator will filter out these changes depending on the platform
	 * info with which the comparator was created.
	 */
	...
	/**
	 * Tests that shows that the same default value expressed differently does
	...
	public void testChangeColumnOrder() {
		...
		assertEquals("TableA", change.getChangedTable());
		assertEquals(3, change.getNewPosition("Col1", true));
		assertEquals(-1, change.getNewPosition("Col2", true));
		assertEquals(1, change.getNewPosition("Col3", true));
	}

	/**
	 * Tests adding a column and changing the order of the existing columns in a
	 * table.
	 */
	public void testAddColumnAndChangeColumnOrder() {
		...
		assertEquals(2, change1.getNewPosition("Col1", false));
		assertEquals(1, change1.getNewPosition("Col2", false));

		assertEquals("TableA", change2.getChangedTable());
		assertColumn("Col3", Types.VARCHAR, "32", null, false, false, false,
				change2.getNewColumn());
		assertEquals("ColPK", change2.getPreviousColumn());
		assertEquals("Col2", change2.getNextColumn());
	}

	/**
	 * Tests removing a column and changing the order of the existing columns in
	 * a table.
	 */
	public void testRemoveColumnAndChangeColumnOrder() {
		...
		assertEquals("TableA", change1.getChangedTable());
		assertEquals("Col3", change1.getChangedColumn());

		assertEquals("TableA", change2.getChangedTable());
		assertEquals(-1, change2.getNewPosition("ColPK", true));
		assertEquals(2, change2.getNewPosition("Col1", true));
		assertEquals(1, change2.getNewPosition("Col2", true));
	}
	...
	public void testDropColumn() {
		...
		assertEquals("TableA", change.getChangedTable());
		assertEquals("Col1", change.getChangedColumn());
	}
