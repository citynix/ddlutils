...
public abstract class TestAgainstLiveDatabaseBase extends TestPlatformBase {
	/**
	 * The name of the property that specifies properties file with the settings
	 * for the connection to test against.
	 */
	...
	public static final String DDLUTILS_PLATFORM_PROPERTY = DDLUTILS_PROPERTY_PREFIX
			+ "platform";
	/** The property specifying the catalog for the tests. */
	public static final String DDLUTILS_CATALOG_PROPERTY = DDLUTILS_PROPERTY_PREFIX
			+ "catalog";
	/** The property specifying the schema for the tests. */
	public static final String DDLUTILS_SCHEMA_PROPERTY = DDLUTILS_PROPERTY_PREFIX
			+ "schema";
	/** The prefix for table creation properties. */
	public static final String DDLUTILS_TABLE_CREATION_PREFIX = DDLUTILS_PROPERTY_PREFIX
			+ "tableCreation.";

	/**
	 * Creates the test suite for the given test class which must be a sub class
	 * of {@link RoundtripTestBase}. If the platform supports it, it will be
	 * tested with both delimited and undelimited identifiers.
	 * 
	 * @param testedClass
	 *            The tested class
	 * @return The tests
	...
	protected static TestSuite getTests(Class testedClass) {
		if (!TestAgainstLiveDatabaseBase.class.isAssignableFrom(testedClass)
			...
			throw new DdlUtilsException(
					"Cannot create parameterized tests for class "
							+ testedClass.getName());
		}
		...
		try {
			...
			for (int idx = 0; (methods != null) && (idx < methods.length); idx++) {
				if (methods[idx].getName().startsWith("test")
						&& ((methods[idx].getParameterTypes() == null) || (methods[idx]
								.getParameterTypes().length == 0))) {
					newTest = (TestAgainstLiveDatabaseBase) testedClass
							.newInstance();
					...
					if (info == null) {
						info = PlatformFactory.createNewPlatformInstance(
								newTest.getDatabaseName()).getPlatformInfo();
					}
	 ...
	 * 
	 * @return The properties or <code>null</code> if no properties have been
	 *         specified
	 */
	protected static Properties readTestProperties() {
		...
		try {
			propStream = TestAgainstLiveDatabaseBase.class
			...
			if (propStream != null) {
				try {
					...
					LogFactory
							.getLog(TestAgainstLiveDatabaseBase.class)
							.error("Could not close the stream used to read the test jdbc properties",
									ex);
				}
			}
		}
	 ...
	 * 
	 * @param props
	 *            The properties to initialize from
			...
			for (Iterator it = props.entrySet().iterator(); it.hasNext();) {
				...
				if (propName.startsWith(DATASOURCE_PROPERTY_PREFIX)
					...
					BeanUtils.setProperty(dataSource, propName
							.substring(DATASOURCE_PROPERTY_PREFIX.length()),
							entry.getValue());
				}
			}
			return dataSource;
	...
	/**
	 * Determines the name of the platform to use from the properties or the
	 * data source if no platform is specified in the properties.
	 * 
	 * @param props
	 *            The test properties
	 * @param dataSource
	...
	protected CreationParameters getTableCreationParameters(Database model) {
		...
		for (Iterator entryIt = _testProps.entrySet().iterator(); entryIt
				.hasNext();) {
	 ...
	 */
	protected Database createDatabase(String schemaXml)
	 ...
	 *            The model
	 */
	protected void createDatabase(Database model)
			throws DatabaseOperationException {
		try {
			_model = model;

			getPlatform().setSqlCommentsOn(false);
			getPlatform().createModel(_model,
					getTableCreationParameters(_model), false, false);
		} catch (Exception ex) {
			throw new DatabaseOperationException(ex);
		}
	}
	 ...
	 */
	protected Database alterDatabase(String schemaXml)
			throws DatabaseOperationException {
		Database model = parseDatabaseFromString(schemaXml);
	 ...
	 *            The model
	 */
	protected void alterDatabase(Database desiredModel)
			throws DatabaseOperationException {
		try {
			_model = desiredModel;
			...
			getPlatform().setSqlCommentsOn(false);
			getPlatform().alterModel(liveModel, _model,
					getTableCreationParameters(_model), false);
		} catch (Exception ex) {
			throw new DatabaseOperationException(ex);
		}
	}
	 ...
	 * 
	 * @param tableName
	 *            The name of the table (case insensitive)
	 * @param columnValues
	 *            The values for the columns in order of definition
	...
	protected DynaBean insertRow(String tableName, Object[] columnValues) {
		...
		for (int idx = 0; (idx < table.getColumnCount())
				&& (idx < columnValues.length); idx++) {
	 ...
	 *            The name of the table (case insensitive)
	 * @param oldBean
	 *            The bean representing the current row
	 * @param columnValues
	 *            The values for the columns in order of definition
	 * @return The dyna bean for the new row
	 */
	protected DynaBean updateRow(String tableName, DynaBean oldBean,
			Object[] columnValues) {
		Table table = getModel().findTable(tableName);
		DynaBean bean = getModel().createDynaBeanFor(table);
	 ...
	 *            The values for the pk columns in order of definition
	 */
	protected void deleteRow(String tableName, Object[] pkColumnValues) {
		...
		for (int idx = 0; (idx < pkColumns.length)
				&& (idx < pkColumnValues.length); idx++) {
	...
	/**
	 * Returns a "SELECT * FROM [table name]" statement. It also takes delimited
	 * identifier mode into account if enabled.
	 * 
	 * @param table
	 *            The table
	 * @param orderColumn
	 *            The column to order the rows by (can be <code>null</code>)
	...
	protected List getRows(String tableName) {
		...
		return getPlatform().fetch(getModel(),
				getSelectQueryForAllString(table, null), new Table[]{table});
	}
	...
	/**
	 * Checks that the database is clear, and if not clears it (no tables,
	 * sequences etc. left) and throws an {@link AssertionFailedError}.
	 */
	protected void assertAndEnsureClearDatabase() {
		...
		if (FirebirdPlatform.DATABASENAME.equals(getPlatform().getName())
				|| InterbasePlatform.DATABASENAME.equals(getPlatform()
						.getName())) {
	...
	private boolean dropGenerators(Connection connection) {
		...
		try {
			...
			ResultSet rs = stmt
					.executeQuery("SELECT RDB$GENERATOR_NAME FROM RDB$GENERATORS WHERE RDB$GENERATOR_NAME NOT LIKE '%$%'");
	...
	private boolean dropTriggers(Connection connection) {
		...
		try {
			...
			ResultSet rs = stmt
					.executeQuery("SELECT * FROM RDB$TRIGGERS WHERE RDB$SYSTEM_FLAG IS NULL OR RDB$SYSTEM_FLAG = 0");
			List names = new ArrayList();
	...
	/**
	 * Returns a copy of the given model adjusted for type changes because of
	 * the native type mappings which when read back from the database will map
	 * to different types.
	 * 
	 * @param sourceModel
	 *            The source model
	...
	protected Database adjustModel(Database sourceModel) {
		...
		for (int tableIdx = 0; tableIdx < model.getTableCount(); tableIdx++) {
			...
			for (int columnIdx = 0; columnIdx < table.getColumnCount(); columnIdx++) {
				...
				// we adjust the column types if the native type would back-map
				// to a
				// different jdbc type
				if (targetType != origType) {
					...
					if (column.getDefaultValue() != null) {
						DefaultValueHelper helper = getPlatform()
								.getSqlBuilder().getDefaultValueHelper();

						column.setDefaultValue(helper.convert(
								column.getDefaultValue(), origType, targetType));
					}
				}
				// we also promote the default size if the column has no size
				// spec of its own
				if ((column.getSize() == null)
						&& getPlatformInfo().hasSize(targetType)) {
					Integer defaultSize = getPlatformInfo().getDefaultSize(
							targetType);
				...
				// finally the platform might return a synthetic default value
				// if the column
			...
			// we also add the default names to foreign keys that are initially
			// unnamed
			for (int fkIdx = 0; fkIdx < table.getForeignKeyCount(); fkIdx++) {
				ForeignKey fk = table.getForeignKey(fkIdx);

				if (fk.getName() == null) {
					fk.setName(getPlatform().getSqlBuilder().getForeignKeyName(
							table, fk));
				}
			}
		}
	...
	/**
	 * Returns the original model adjusted for type changes because of the
	 * native type mappings which when read back from the database will map to
	 * different types.
	 * 
	 * @return The adjusted model
	 */
	...
	/**
	 * Returns the SQL for altering the live database so that it matches the
	 * given model.
	 * 
	 * @param desiredModel
	 *            The desired model
	...
	protected String getAlterTablesSql(Database desiredModel) {
		...
		return getPlatform().getAlterModelSql(liveModel, desiredModel,
				getTableCreationParameters(desiredModel));
	}
	...
	/**
	 * Asserts that the two given database models are equal, and if not, writes
	 * both of them in XML form to <code>stderr</code>.
	 * 
	 * @param expected
	 *            The expected model
	 * @param actual
	...
	/**
	 * Compares the specified attribute value of the given bean with the
	 * expected object.
	 * 
	 * @param expected
	 *            The expected object
	...
	protected void assertEquals(Object expected, Object bean, String attrName) {
		...
		if ((value instanceof byte[]) && !(expected instanceof byte[])
			...
			SqlDynaClass dynaClass = (SqlDynaClass) ((SqlDynaBean) dynaBean)
					.getDynaClass();
			Column column = ((SqlDynaProperty) dynaClass
					.getDynaProperty(attrName)).getColumn();
	 ...
	 *            The actual model
	 * @param caseSensitive
	 *            Whether case matters when comparing
	 */
	protected void assertEquals(Database expected, Database actual,
			boolean caseSensitive) {
		try {
			assertEquals("Model names do not match.", expected.getName(),
					actual.getName());
			assertEquals("Not the same number of tables.",
					expected.getTableCount(), actual.getTableCount());
			for (int tableIdx = 0; tableIdx < actual.getTableCount(); tableIdx++) {
				assertEquals(expected.getTable(tableIdx),
						actual.getTable(tableIdx), caseSensitive);
			}
		} catch (Throwable ex) {
	...
	protected void assertEquals(Table expected, Table actual,
		...
		if (caseSensitive) {
			assertEquals(
					...
					getPlatform().getSqlBuilder().shortenName(
							expected.getName(),
			...
			assertEquals(
					"Table names do not match (ignoring case).",
					getPlatform().getSqlBuilder().shortenName(
		...
		assertEquals(
				"Not the same number of columns in table " + actual.getName()
		...
		for (int columnIdx = 0; columnIdx < actual.getColumnCount(); columnIdx++) {
			assertEquals(expected.getColumn(columnIdx),
					actual.getColumn(columnIdx), caseSensitive);
		}
		assertEquals(
				"Not the same number of foreign keys in table "
						+ actual.getName() + ".",
				expected.getForeignKeyCount(), actual.getForeignKeyCount());
		// order is not assumed with the way foreignkeys are returned.
		for (int expectedFkIdx = 0; expectedFkIdx < expected
				.getForeignKeyCount(); expectedFkIdx++) {
			ForeignKey expectedFk = expected.getForeignKey(expectedFkIdx);
			String expectedName = getPlatform().getSqlBuilder().shortenName(
					expectedFk.getName(),
			...
			for (int actualFkIdx = 0; actualFkIdx < actual.getForeignKeyCount(); actualFkIdx++) {
				...
				String actualName = getPlatform().getSqlBuilder().shortenName(
						actualFk.getName(),
						getSqlBuilder().getMaxForeignKeyNameLength());

				if (StringUtilsExt.equals(expectedName, actualName,
						caseSensitive)) {
		...
		assertEquals(
				"Not the same number of indices in table " + actual.getName()
						+ ".", expected.getIndexCount(), actual.getIndexCount());
		for (int indexIdx = 0; indexIdx < actual.getIndexCount(); indexIdx++) {
			assertEquals(expected.getIndex(indexIdx),
					actual.getIndex(indexIdx), caseSensitive);
		}
	}
	...
	protected void assertEquals(Column expected, Column actual,
		...
		if (caseSensitive) {
			...
			assertEquals(
					"Column names do not match (ignoring case).",
					getPlatform().getSqlBuilder().shortenName(
							expected.getName().toUpperCase(),
							getSqlBuilder().getMaxColumnNameLength()),
					getPlatform().getSqlBuilder().shortenName(
		...
		assertEquals(
				"Primary key status not the same for column "
						+ actual.getName() + ".", expected.isPrimaryKey(),
				actual.isPrimaryKey());
		assertEquals(
				"Required status not the same for column " + actual.getName()
						+ ".", expected.isRequired(), actual.isRequired());
		if (getPlatformInfo().getIdentityStatusReadingSupported()) {
			...
			assertEquals("Auto-increment status not the same for column "
					+ actual.getName() + ".", expected.isAutoIncrement(),
		...
		assertEquals("Type code not the same for column " + actual.getName()
				+ ".", expected.getTypeCode(), actual.getTypeCode());
		assertEquals(
				"Parsed default values do not match for column "
						+ actual.getName() + ".",
		...
		if ((expected.getTypeCode() == Types.NUMERIC)
			...
			assertEquals(
					"Precision not the same for column " + actual.getName()
	...
	protected void assertEquals(ForeignKey expected, ForeignKey actual,
		...
		if (caseSensitive) {
			assertEquals(
					"Foreign key names do not match.",
					getPlatform().getSqlBuilder().shortenName(
							expected.getName(),
							getSqlBuilder().getMaxForeignKeyNameLength()),
					getPlatform().getSqlBuilder().shortenName(actual.getName(),
							getSqlBuilder().getMaxForeignKeyNameLength()));
			assertEquals(
					"Referenced table names do not match.",
					getPlatform().getSqlBuilder().shortenName(
			...
			assertEquals(
					"Foreign key names do not match (ignoring case).",
					getPlatform().getSqlBuilder().shortenName(
							expected.getName().toUpperCase(),
							getSqlBuilder().getMaxForeignKeyNameLength()),
					getPlatform().getSqlBuilder().shortenName(
							...
							getSqlBuilder().getMaxForeignKeyNameLength()));
			assertEquals(
					"Referenced table names do not match (ignoring case).",
					getPlatform().getSqlBuilder().shortenName(
		...
		assertTrue(
				"Not the same onUpdate setting in foreign key "
						+ actual.getName() + ": expected = "
						+ expected.getOnUpdate() + ", actual = "
						+ actual.getOnUpdate(),
				expected.getOnUpdate().equals(actual.getOnUpdate())
						|| getPlatformInfo().areEquivalentOnUpdateActions(
								expected.getOnUpdate(), actual.getOnUpdate()));
		assertTrue(
				"Not the same onDelete setting in foreign key "
						+ actual.getName() + ": expected = "
						+ expected.getOnDelete() + ", actual = "
						+ actual.getOnDelete(),
				expected.getOnDelete().equals(actual.getOnDelete())
						|| getPlatformInfo().areEquivalentOnDeleteActions(
								expected.getOnDelete(), actual.getOnDelete()));

		assertEquals("Not the same number of references in foreign key "
				+ actual.getName() + ".", expected.getReferenceCount(),
				actual.getReferenceCount());
		for (int refIdx = 0; refIdx < actual.getReferenceCount(); refIdx++) {
			assertEquals(expected.getReference(refIdx),
					actual.getReference(refIdx), caseSensitive);
		}
	}
	...
	protected void assertEquals(Reference expected, Reference actual,
		...
		if (caseSensitive) {
			assertEquals(
					"Local column names do not match.",
					getPlatform().getSqlBuilder().shortenName(
					...
					getPlatform().getSqlBuilder().shortenName(
							...
							getSqlBuilder().getMaxColumnNameLength()));
			assertEquals(
					"Foreign column names do not match.",
					getPlatform().getSqlBuilder().shortenName(
			...
			assertEquals(
					"Local column names do not match (ignoring case).",
					getPlatform().getSqlBuilder().shortenName(
					...
					getPlatform().getSqlBuilder().shortenName(
							...
							getSqlBuilder().getMaxColumnNameLength()));
			assertEquals(
					"Foreign column names do not match (ignoring case).",
					getPlatform().getSqlBuilder().shortenName(
	...
	protected void assertEquals(Index expected, Index actual,
		...
		if (caseSensitive) {
			...
			assertEquals(
					"Index names do not match (ignoring case).",
					getPlatform().getSqlBuilder().shortenName(
							expected.getName().toUpperCase(),
							getSqlBuilder().getMaxConstraintNameLength()),
					getPlatform().getSqlBuilder().shortenName(
		...
		assertEquals(
				"Not the same number of columns in index " + actual.getName()
						+ ".", expected.getColumnCount(),
				actual.getColumnCount());
		for (int columnIdx = 0; columnIdx < actual.getColumnCount(); columnIdx++) {
			assertEquals(expected.getColumn(columnIdx),
	...
	protected void assertEquals(IndexColumn expected, IndexColumn actual,
		...
		if (caseSensitive) {
			assertEquals(
					"Index column names do not match.",
					getPlatform().getSqlBuilder().shortenName(
							expected.getName(),
							getSqlBuilder().getMaxColumnNameLength()),
					getPlatform().getSqlBuilder().shortenName(actual.getName(),
			...
			assertEquals(
					"Index column names do not match (ignoring case).",
					getPlatform().getSqlBuilder().shortenName(
							expected.getName().toUpperCase(),
							getSqlBuilder().getMaxColumnNameLength()),
					getPlatform().getSqlBuilder().shortenName(
