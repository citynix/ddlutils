 ...
 * 
 * TODO: add more tests, esp. combining multiple changes - change datatype/size
 * and add to/remove from pk - change datatype/size and add/remove pk that uses
 * the column - change type of column in index and foreign key - drop index with
 * columns in a foreign key - ...
 * 
 * @version $Revision: $
 */
public class TestAlteration extends TestAgainstLiveDatabaseBase {
	...
	/**
	 * Tests the removal of an index that has column that are also used by
	 * foreign keys. This is a test esp. for the handling of
	...
	public void testDropIndexOverlappingWithForeignKeys() {
		...
		insertRow("roundtrip3", new Object[]{new Integer(1), new Integer(1),
				"test"});
	...
	/**
	 * Tests the removal of an index that has column that are also referenced by
	 * a remote foreign key.
	 */
	...
	/**
	 * Tests removing a foreign key and an index that has the same name and same
	 * column.
	 */
	public void testDropFKAndCorrespondingIndex() {
		if (!getPlatformInfo().isIndicesSupported()
				|| FirebirdPlatform.DATABASENAME
						.equals(getPlatform().getName())) {
			// Firebird does not allow an index and a foreign key in the same
			// table to have the same name
			return;
		}
	...
	/**
	 * Tests removing a foreign key but not the index that has the same name and
	 * same column.
	 */
	...
	/**
	 * Tests removing a foreign key and an index that has the same name but
	 * different columns.
	 */
	public void testDropFKAndDifferentIndexWithSameName() {
		// MySql/InnoDB doesn't allow the creation of a foreign key and index
		// with the same name
		// unless the index can be used as the FK's index
		// Firebird does not allow an index and a foreign key in the same table
		// to have the same name at all
		if (!getPlatformInfo().isIndicesSupported()
	...
	/**
	 * Tests removing a foreign key but not the index that has the same name but
	 * different columns.
	 */
	...
	public void testDropFKs() {
		...
		insertRow("roundtrip4", new Object[]{new Integer(1), new Integer(3),
				new Integer(2)});
	...
	public void testRemoveReferenceFromFK() {
		...
		insertRow("roundtrip1", new Object[]{new Integer(1), "test"});
		insertRow("roundtrip2", new Object[]{new Integer(2), "test",
				new Integer(1)});
	...
	public void testAddTable2() {
		...
		final String model2Xml = "<?xml version='1.0' encoding='ISO-8859-1'?>\n"
				...
				+ "    <column name='avalue' type='INTEGER'/>\n"
				+ "  </table>\n" + "</database>";
		final String model3Xml = "<?xml version='1.0' encoding='ISO-8859-1'?>\n"
				+ "<database xmlns='"
				+ DatabaseIO.DDLUTILS_NAMESPACE
				+ "' name='roundtriptest'>\n"
				+ "  <table name='roundtrip1'>\n"
		...
		// note that we have to split the alteration because we can only add the
		// foreign key if
	...
	public void testIssue54() throws Exception {
		...
		String alterationSql = getPlatform().getAlterTablesSql(catalog, schema,
				null, model);
	...
	public void testRenamePK() throws Exception {
		final String model1Xml = "<?xml version='1.0'?>\n"
				+ "<database xmlns='"
				+ DatabaseIO.DDLUTILS_NAMESPACE
				+ "' name='roundtriptest'>\n"
				+ "  <table name='roundtrip'>\n"
				...
				+ "  </table>\n" + "</database>";
		final String model2Xml = "<?xml version='1.0'?>\n"
				+ "<database xmlns='"
				+ DatabaseIO.DDLUTILS_NAMESPACE
				+ "' name='roundtriptest'>\n"
				+ "  <table name='roundtrip'>\n"
		...
		if (MckoiPlatform.DATABASENAME.equals(getPlatform().getName())) {
			// McKoi can actually handle this, though interestingly it will
			// result in a null value for the pk
