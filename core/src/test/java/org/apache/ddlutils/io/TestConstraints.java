...
public class TestConstraints extends TestAgainstLiveDatabaseBase {
	...
	/**
	 * Tests a nullable column. Basically we're creating the test database and
	 * then read it back and compare the original with the read one. In addition
	 * we can also check that DdlUtils does not try to alter the new database
	 * when using the <code>alterTables</code>/<code>getAlterTablesSql</code>
	 * methods of the {@link org.apache.ddlutils.Platform} with the read-back
	 * model.
	 * 
	 * @param modelXml
	 *            The model to be tested in XML form
	 * @param checkAlteration
	 *            Whether to also check the alter tables sql
	 */
	protected void performConstraintsTest(String modelXml,
			boolean checkAlteration) {
	...
	public void testLongTableName() {
		...
		String tableName = StringUtils.repeat("Test", (getSqlBuilder()
				.getMaxTableNameLength() / 4) + 3);
	...
	public void testLongColumnName() {
		...
		String columnName = StringUtils.repeat("Test", (getSqlBuilder()
				.getMaxColumnNameLength() / 4) + 3);
		final String modelXml = "<?xml version='1.0' encoding='ISO-8859-1'?>\n"
				+ "<database xmlns='"
				+ DatabaseIO.DDLUTILS_NAMESPACE
				+ "' name='roundtriptest'>\n"
	...
	public void testLongConstraintName() {
		...
		String constraintName = StringUtils.repeat("Test", (getSqlBuilder()
				.getMaxConstraintNameLength() / 4) + 3);
		final String modelXml = "<?xml version='1.0' encoding='ISO-8859-1'?>\n"
				+ "<database xmlns='"
				+ DatabaseIO.DDLUTILS_NAMESPACE
				+ "' name='roundtriptest'>\n"
	...
	public void testLongForeignKeyName() {
		...
		String fkName = StringUtils.repeat("Test", (getSqlBuilder()
				.getMaxForeignKeyNameLength() / 4) + 3);
		final String modelXml = "<?xml version='1.0' encoding='ISO-8859-1'?>\n"
				+ "<database xmlns='"
				+ DatabaseIO.DDLUTILS_NAMESPACE
				+ "' name='roundtriptest'>\n"
	...
	public void testAutoIncrementIntegerColumn() {
		...
		performConstraintsTest(modelXml, getPlatformInfo()
	...
	public void testForeignKeyWithOnDeleteRestrict() {
		if (!getPlatformInfo().isActionSupportedForOnDelete(
	...
	public void testForeignKeyWithOnUpdateRestrict() {
		if (!getPlatformInfo().isActionSupportedForOnUpdate(
				CascadeActionEnum.RESTRICT)) {
			return;
		}
		...
		try {
			updateRow("roundtrip_1", (DynaBean) beansTable1.get(0),
	...
	public void testForeignKeyWithOnUpdateSetDefault() {
		...
		updateRow("roundtrip_1", (DynaBean) beansTable1.get(1),
