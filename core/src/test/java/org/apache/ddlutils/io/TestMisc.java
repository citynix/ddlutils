...
public class TestMisc extends TestAgainstLiveDatabaseBase {
	...
	/**
	 * Tests the backup and restore of a table with an identity column and a
	 * foreign key to it when identity override is turned on.
	 */
	public void testIdentityOverrideOn() throws Exception {
		...
		Document testDoc = reader.read(new InputSource(new StringReader(
				dataAsXml)));
		...
		assertEquals("10",
				((Element) misc1Rows.get(0)).attributeValue(pkColumnName));
		assertEquals("1",
				((Element) misc1Rows.get(0)).attributeValue(valueColumnName));
		assertEquals("12",
				((Element) misc1Rows.get(1)).attributeValue(pkColumnName));
		assertEquals("2",
				((Element) misc1Rows.get(1)).attributeValue(valueColumnName));
		assertEquals("13",
				((Element) misc1Rows.get(2)).attributeValue(pkColumnName));
		assertEquals("3",
				((Element) misc1Rows.get(2)).attributeValue(valueColumnName));
		assertEquals(2, misc2Rows.size());
		assertEquals("1",
				((Element) misc2Rows.get(0)).attributeValue(pkColumnName));
		assertEquals("10",
				((Element) misc2Rows.get(0)).attributeValue(fkColumnName));
		assertEquals("2",
				((Element) misc2Rows.get(1)).attributeValue(pkColumnName));
		assertEquals("13",
				((Element) misc2Rows.get(1)).attributeValue(fkColumnName));
	...
	/**
	 * Tests the backup and restore of a table with an identity column and a
	 * foreign key to it when identity override is turned off.
	 */
	...
	/**
	 * Tests the backup and restore of a table with an identity column and a
	 * foreign key to itself while identity override is off.
	 */
	public void testSelfReferenceIdentityOverrideOff() throws Exception {
		...
		if (isSybase) {
			...
			insertRow("misc",
					new Object[]{new BigDecimal(2), new BigDecimal(1)});
			insertRow("misc",
					new Object[]{new BigDecimal(3), new BigDecimal(2)});
			insertRow("misc",
					new Object[]{new BigDecimal(4), new BigDecimal(4)});
		} else {
		...
		assertEquals("1",
				((Element) miscRows.get(0)).attributeValue(pkColumnName));
		assertNull(((Element) miscRows.get(0)).attributeValue(fkColumnName));
		assertEquals("2",
				((Element) miscRows.get(1)).attributeValue(pkColumnName));
		assertEquals("1",
				((Element) miscRows.get(1)).attributeValue(fkColumnName));
		assertEquals("3",
				((Element) miscRows.get(2)).attributeValue(pkColumnName));
		assertEquals("2",
				((Element) miscRows.get(2)).attributeValue(fkColumnName));
		assertEquals("4",
				((Element) miscRows.get(3)).attributeValue(pkColumnName));
		assertEquals("4",
				((Element) miscRows.get(3)).attributeValue(fkColumnName));
	...
	public void testSelfReferenceIdentityOverrideOn() throws Exception {
		...
		if (isSybase) {
			...
			insertRow("misc", new Object[]{new BigDecimal(11),
					new BigDecimal(10)});
			insertRow("misc", new Object[]{new BigDecimal(12),
					new BigDecimal(11)});
			insertRow("misc", new Object[]{new BigDecimal(13),
					new BigDecimal(13)});
		} else {
	...
	public void testSelfReferences() throws Exception {
		...
		final String dataXml = "<?xml version='1.0' encoding='ISO-8859-1'?>\n"
	...
	/**
	 * Tests the backup and restore of a self-referencing data set (with
	 * multiple self references in the same table).
	 */
	 ...
	 * with an identity column and a foreign key to itself while identity
	 * override is off.
	 */
	public void testComplexTableModel() throws Exception {
		...
		insertRow("B", new Object[]{new Integer(1), new Integer(2),
				new Integer(1)});
		...
		HashSet expectedValues = new HashSet(Arrays.asList(new String[]{"1",
				"2"}));
	...
	public void testDdlUtils227() throws Exception {
		...
		Table table = getModel().findTable("Roundtrip",
		...
		query.append(")");
		// Some JDBC drivers do not allow us to perform the query without an
		// explicit alias
		if (MySqlPlatform.DATABASENAME.equals(getPlatform().getName())
				|| MySql50Platform.DATABASENAME.equals(getPlatform().getName())
				|| PostgreSqlPlatform.DATABASENAME.equals(getPlatform()
						.getName())
		...
		beans = getPlatform().fetch(getModel(), query.toString(),
				new Table[]{table});
