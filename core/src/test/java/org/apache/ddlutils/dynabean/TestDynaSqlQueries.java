...
public class TestDynaSqlQueries extends TestAgainstLiveDatabaseBase {
	...
	/**
	 * Helper method to wrap the given identifier in delimiters if delimited
	 * identifier mode is turned on for the test.
	 * 
	 * @param name
	 *            The identifier
	 * @return The identifier, wrapped if delimited identifier mode is turned
	 *         on, or as-is if not
	 */
	private String asIdentifier(String name) {
		if (getPlatform().isDelimitedIdentifierModeOn()) {
			return getPlatformInfo().getDelimiterToken() + name
	...
	public void testSimpleQuery() throws Exception {
		...
		ModelBasedResultSetIterator it = (ModelBasedResultSetIterator) getPlatform()
				.query(getModel(),
	...
	/**
	 * Tests a more complicated SELECT query that leads to a JOIN in the
	 * database.
	 */
	...
	public void testInsertSingle() throws Exception {
		...
		SqlDynaClass dynaClass = SqlDynaClass.newInstance(getModel()
				.getTable(0));
