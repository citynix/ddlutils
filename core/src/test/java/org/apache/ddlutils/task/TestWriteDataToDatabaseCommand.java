...
public class TestWriteDataToDatabaseCommand extends TestTaskBase {
	...
	/**
	 * Adds the writeDataToDatabase sub task to the given task, executes it, and
	 * checks its output.
	 * 
	 * @param task
	 *            The task
	 * @param dataXml
	 *            The data xml to write
	 * @param useBatchMode
	 *            Whether to use batch mode for inserting the data
	 * @param ensureFkOrder
	 *            Whether to ensure foreign key order
	 */
	private void runTask(DatabaseToDdlTask task, String dataXml,
			boolean useBatchMode, boolean ensureFkOrder) throws IOException {
		...
		try {
			...
			if (writer != null) {
				try {
					...
					getLog().error(
							"Could not close the writer for the temporary file "
