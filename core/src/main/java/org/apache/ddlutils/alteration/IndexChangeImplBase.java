...
public abstract class IndexChangeImplBase extends TableChangeImplBase
	 ...
	 * @param index
	 *            The index; note that this change object will not maintain a
			...
			for (int indexIdx = 0; indexIdx < table.getIndexCount(); indexIdx++) {
				...
				if (curIndex.getColumnCount() == _columnNames.size()) {
					for (int colIdx = 0; colIdx < curIndex.getColumnCount(); colIdx++) {
						String curColName = curIndex.getColumn(colIdx)
								.getName();
						String expectedColName = (String) _columnNames
								.get(colIdx);

						if ((caseSensitive && curColName
								.equals(expectedColName))
								|| (!caseSensitive && curColName
										.equalsIgnoreCase(expectedColName))) {
							return curIndex;
						}
					}
				}
