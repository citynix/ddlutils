...
public class ModelComparator {
	...
	/**
	 * Whether {@link RemoveColumnChange} objects for primary key columns are
	 * enough or additional primary key change objects are necessary.
	 */
	 ...
	 * @param platformInfo
	 *            The platform info
	 * @param tableDefChangePredicate
	 *            The predicate that defines whether tables changes are
	 *            supported by the platform or not; all changes are supported if
	 *            this is null
	 * @param caseSensitive
	 *            Whether comparison is case sensitive
	 */
	...
	/**
	 * Specifies whether the comparator should generate {@link PrimaryKeyChange}
	 * objects or a pair of {@link RemovePrimaryKeyChange} and
	 * {@link AddPrimaryKeyChange} objects instead. The default value is
	 * <code>true</code>.
	 * 
	 * @param generatePrimaryKeyChanges
	 *            Whether to create {@link PrimaryKeyChange} objects
	 */
	 ...
	 * primary key columns. If the platform cannot drop primary key columns, set
	 * this to <code>false</code> and the comparator will create additional
	 * primary key changes. The default value is <code>true</code>.
	 * 
	 * @param canDropPrimaryKeyColumns
	 *            Whether {@link RemoveColumnChange} objecs for primary key
	 *            columns are ok
	 */
	...
	/**
	 * Compares the given source and target models and creates change objects to
	 * get from the source to the target one. These changes will be applied to
	 * the given intermediate model (the other two won't be changed), so that it
	 * will be equal to the target model after this model has finished.
	 * 
	 * @param sourceModel
	 *            The source model
	 * @param intermediateModel
	 *            The intermediate model to apply the changes to
	 * @param targetModel
	 *            The target model
	 * @return The changes
	 */
	protected List compareModels(Database sourceModel,
			Database intermediateModel, Database targetModel) {
		ArrayList changes = new ArrayList();

		changes.addAll(checkForRemovedForeignKeys(sourceModel,
		...
		for (int tableIdx = 0; tableIdx < intermediateModel.getTableCount(); tableIdx++) {
			...
			Table sourceTable = sourceModel.findTable(
					intermediateTable.getName(), _caseSensitive);
			Table targetTable = targetModel.findTable(
					intermediateTable.getName(), _caseSensitive);
			List tableChanges = compareTables(sourceModel, sourceTable,
					intermediateModel, intermediateTable, targetModel,
					targetTable);

			changes.addAll(tableChanges);
		}

		changes.addAll(checkForAddedTables(sourceModel, intermediateModel,
				targetModel));
		changes.addAll(checkForAddedForeignKeys(sourceModel, intermediateModel,
				targetModel));
		return changes;
	}

	/**
	 * Creates change objects for foreign keys that are present in the given
	 * source model but are no longer in the target model, and applies them to
	 * the given intermediate model.
	 * 
	 * @param sourceModel
	 *            The source model
	 * @param intermediateModel
	...
	protected List checkForRemovedForeignKeys(Database sourceModel,
		...
		for (int tableIdx = 0; tableIdx < intermediateModel.getTableCount(); tableIdx++) {
			...
			// Dropping foreign keys from tables to be removed might not be
			// necessary, but some databases might require it
			for (int fkIdx = 0; fkIdx < intermediateFks.length; fkIdx++) {
				ForeignKey sourceFk = intermediateFks[fkIdx];
				ForeignKey targetFk = targetTable == null
						? null
						: findCorrespondingForeignKey(targetTable, sourceFk);

				if (targetFk == null) {
					if (_log.isInfoEnabled()) {
						_log.info("Foreign key " + sourceFk
								+ " needs to be removed from table "
								+ intermediateTable.getName());
					}

					RemoveForeignKeyChange fkChange = new RemoveForeignKeyChange(
							intermediateTable.getName(), sourceFk);
	...
	/**
	 * Creates change objects for foreign keys that are not present in the given
	 * source model but are in the target model, and applies them to the given
	 * intermediate model.
	 * 
	 * @param sourceModel
	 *            The source model
	...
	protected List checkForAddedForeignKeys(Database sourceModel,
		...
		for (int tableIdx = 0; tableIdx < targetModel.getTableCount(); tableIdx++) {
			...
			Table intermediateTable = intermediateModel.findTable(
					targetTable.getName(), _caseSensitive);

			for (int fkIdx = 0; fkIdx < targetTable.getForeignKeyCount(); fkIdx++) {
				ForeignKey targetFk = targetTable.getForeignKey(fkIdx);
				ForeignKey intermediateFk = findCorrespondingForeignKey(
						intermediateTable, targetFk);

				if (intermediateFk == null) {
					if (_log.isInfoEnabled()) {
						_log.info("Foreign key " + targetFk
								+ " needs to be added to table "
								+ intermediateTable.getName());
					}

					intermediateFk = _cloneHelper.clone(targetFk,
							intermediateTable, intermediateModel,
							_caseSensitive);

					AddForeignKeyChange fkChange = new AddForeignKeyChange(
							intermediateTable.getName(), intermediateFk);
	...
	/**
	 * Creates change objects for tables that are present in the given source
	 * model but are no longer in the target model, and applies them to the
	 * given intermediate model.
	 * 
	 * @param sourceModel
	 *            The source model
	 * @param intermediateModel
	...
	protected List checkForRemovedTables(Database sourceModel,
		...
		for (int tableIdx = 0; tableIdx < intermediateTables.length; tableIdx++) {
			...
			if (targetTable == null) {
				if (_log.isInfoEnabled()) {
					_log.info("Table " + intermediateTable.getName()
							+ " needs to be removed");
				}

				RemoveTableChange tableChange = new RemoveTableChange(
						intermediateTable.getName());
	...
	/**
	 * Creates change objects for tables that are not present in the given
	 * source model but are in the target model, and applies them to the given
	 * intermediate model.
	 * 
	 * @param sourceModel
	...
	protected List checkForAddedTables(Database sourceModel,
		...
		for (int tableIdx = 0; tableIdx < targetModel.getTableCount(); tableIdx++) {
			...
			if (intermediateTable == null) {
				if (_log.isInfoEnabled()) {
					_log.info("Table " + targetTable.getName()
							+ " needs to be added");
				}

				// we're using a clone of the target table, and remove all
				// foreign
				// keys as these will be added later
				intermediateTable = _cloneHelper.clone(targetTable, true,
						false, intermediateModel, _caseSensitive);

				AddTableChange tableChange = new AddTableChange(
						intermediateTable);
	 ...
	 *            The source table
	 * @param intermediateModel
	 *            The intermediate model to which the changes will be applied
	 *            incrementally
	 * @param intermediateTable
	 *            The table corresponding to the source table in the
	 *            intermediate model
	 * @param targetModel
	 ...
	 * @return The changes
	 */
	protected List compareTables(Database sourceModel, Table sourceTable,
			Database intermediateModel, Table intermediateTable,
			Database targetModel, Table targetTable) {
		ArrayList changes = new ArrayList();

		changes.addAll(checkForRemovedIndexes(sourceModel, sourceTable,
				intermediateModel, intermediateTable, targetModel, targetTable));

		ArrayList tableDefinitionChanges = new ArrayList();
		Table tmpTable = _cloneHelper.clone(intermediateTable, true, false,
				intermediateModel, _caseSensitive);

		tableDefinitionChanges.addAll(checkForRemovedColumns(sourceModel,
				sourceTable, intermediateModel, intermediateTable, targetModel,
				targetTable));
		tableDefinitionChanges.addAll(checkForChangeOfColumnOrder(sourceModel,
				sourceTable, intermediateModel, intermediateTable, targetModel,
				targetTable));
		tableDefinitionChanges.addAll(checkForChangedColumns(sourceModel,
				sourceTable, intermediateModel, intermediateTable, targetModel,
				targetTable));
		...
		if (!tableDefinitionChanges.isEmpty()) {
			if ((_tableDefCangePredicate == null)
					|| _tableDefCangePredicate.areSupported(tmpTable,
							tableDefinitionChanges)) {
				changes.addAll(tableDefinitionChanges);
			} else {
				// we need to recreate the table; for this to work we need to
				// remove foreign keys to and from the table
				// however, we don't have to add them back here as there is a
				// check for added foreign keys/indexes
				// later on anyways
				...
				for (int fkIdx = 0; fkIdx < fks.length; fkIdx++) {
					RemoveForeignKeyChange fkChange = new RemoveForeignKeyChange(
							intermediateTable.getName(), fks[fkIdx]);

					changes.add(fkChange);
					fkChange.apply(intermediateModel, _caseSensitive);
				}
				for (int tableIdx = 0; tableIdx < intermediateModel
						.getTableCount(); tableIdx++) {
					...
					if (curTable != intermediateTable) {
						...
						for (int fkIdx = 0; fkIdx < curFks.length; fkIdx++) {
							if ((_caseSensitive && curFks[fkIdx]
									.getForeignTableName().equals(
											intermediateTable.getName()))
									|| (!_caseSensitive && curFks[fkIdx]
											.getForeignTableName()
											.equalsIgnoreCase(
													intermediateTable.getName()))) {
								RemoveForeignKeyChange fkChange = new RemoveForeignKeyChange(
										curTable.getName(), curFks[fkIdx]);

								changes.add(fkChange);
								fkChange.apply(intermediateModel,
										_caseSensitive);
							}
						}
					}
				...
				RecreateTableChange tableChange = new RecreateTableChange(
						intermediateTable.getName(), intermediateTable,
		...
		changes.addAll(checkForAddedIndexes(sourceModel, sourceTable,
				intermediateModel, intermediateTable, targetModel, targetTable));
	...
	/**
	 * Returns the names of the columns in the intermediate table corresponding
	 * to the given column objects.
	 * 
	 * @param columns
	 *            The column objects
	 * @param intermediateTable
	 ...
	 */
	protected String[] getIntermediateColumnNamesFor(Column[] columns,
			Table intermediateTable) {
		String[] result = new String[columns.length];

		for (int idx = 0; idx < columns.length; idx++) {
			result[idx] = intermediateTable.findColumn(columns[idx].getName(),
					_caseSensitive).getName();
		}
		return result;
	}

	/**
	 * Creates change objects for indexes that are present in the given source
	 * table but are no longer in the target table, and applies them to the
	 * given intermediate model.
	 * 
	 * @param sourceModel
	 *            The source model
	 ...
	 *            The intermediate model to apply the changes to
	 * @param intermediateTable
	 *            The table from the intermediate model corresponding to the
	 *            source table
	 * @param targetModel
	 *            The target model
	 * @param targetTable
	...
	protected List checkForRemovedIndexes(Database sourceModel,
			Table sourceTable, Database intermediateModel,
			Table intermediateTable, Database targetModel, Table targetTable) {
		List changes = new ArrayList();
		...
		for (int indexIdx = 0; indexIdx < indexes.length; indexIdx++) {
			...
			if (targetIndex == null) {
				if (_log.isInfoEnabled()) {
					_log.info("Index " + sourceIndex.getName()
							+ " needs to be removed from table "
							+ intermediateTable.getName());
				}

				RemoveIndexChange change = new RemoveIndexChange(
						intermediateTable.getName(), sourceIndex);
	...
	/**
	 * Creates change objects for indexes that are not present in the given
	 * source table but are in the target table, and applies them to the given
	 * intermediate model.
	 * 
	 * @param sourceModel
	 *            The source model
	...
	protected List checkForAddedIndexes(Database sourceModel,
		...
		for (int indexIdx = 0; indexIdx < targetTable.getIndexCount(); indexIdx++) {
			...
			Index intermediateIndex = findCorrespondingIndex(intermediateTable,
					targetIndex);
			Index sourceIndex = findCorrespondingIndex(sourceTable, targetIndex);

			if ((sourceIndex == null) && (intermediateIndex == null)) {
				if (_log.isInfoEnabled()) {
					_log.info("Index " + targetIndex.getName()
							+ " needs to be created for table "
							+ intermediateTable.getName());
				}

				Index clonedIndex = _cloneHelper.clone(targetIndex,
						intermediateTable, _caseSensitive);
				AddIndexChange change = new AddIndexChange(
						intermediateTable.getName(), clonedIndex);
	...
	/**
	 * Checks for changes in the column order between the given source and
	 * target table, creates change objects for these and applies them to the
	 * given intermediate model.
	 * 
	 * @param sourceModel
	 *            The source model
	...
	protected List checkForChangeOfColumnOrder(Database sourceModel,
		...
		for (int columnIdx = 0; columnIdx < targetTable.getColumnCount(); columnIdx++) {
			...
			Column sourceColumn = intermediateTable.findColumn(
		...
		for (int columnIdx = 0; columnIdx < intermediateTable.getColumnCount(); columnIdx++) {
			...
			if ((targetIdx >= 0) && (targetIdx != columnIdx)) {
				newPositions
						.put(sourceColumn.getName(), new Integer(targetIdx));
		...
		if (!newPositions.isEmpty()) {
			ColumnOrderChange change = new ColumnOrderChange(
					intermediateTable.getName(), newPositions);

			change.apply(intermediateModel, _caseSensitive);
			if (numChangedPKs > 1) {
				// create pk change that only covers the order change
				// fortunately, the order change will have adjusted the pk order
				// already
	...
	/**
	 * Creates change objects for columns that are present in the given source
	 * table but are no longer in the target table, and applies them to the
	 * given intermediate model.
	 * 
	 * @param sourceModel
	...
	protected List checkForRemovedColumns(Database sourceModel,
			Table sourceTable, Database intermediateModel,
			Table intermediateTable, Database targetModel, Table targetTable) {
		// if the platform does not support dropping pk columns, then the pk
		// handling above will
		...
		for (int columnIdx = 0; columnIdx < columns.length; columnIdx++) {
			...
			if (targetColumn == null) {
				if (_log.isInfoEnabled()) {
					_log.info("Column " + sourceColumn.getName()
							+ " needs to be removed from table "
							+ intermediateTable.getName());
				}

				RemoveColumnChange change = new RemoveColumnChange(
						intermediateTable.getName(), sourceColumn.getName());
	...
	/**
	 * Creates change objects for columns that are not present in the given
	 * source table but are in the target table, and applies them to the given
	 * intermediate model.
	 * 
	 * @param sourceModel
	...
	protected List checkForAddedColumns(Database sourceModel,
		...
		for (int columnIdx = 0; columnIdx < targetTable.getColumnCount(); columnIdx++) {
			...
			if (sourceColumn == null) {
				String prevColumn = (columnIdx > 0 ? intermediateTable
						.getColumn(columnIdx - 1).getName() : null);
				String nextColumn = (columnIdx < intermediateTable
						.getColumnCount() ? intermediateTable.getColumn(
						columnIdx).getName() : null);
				Column clonedColumn = _cloneHelper.clone(targetColumn, false);
				AddColumnChange change = new AddColumnChange(
						intermediateTable.getName(), clonedColumn, prevColumn,
						nextColumn);
	...
	/**
	 * Creates change objects for columns that have a different in the given
	 * source and target table, and applies them to the given intermediate
	 * model.
	 * 
	 * @param sourceModel
	 *            The source model
	...
	protected List checkForChangedColumns(Database sourceModel,
		...
		for (int columnIdx = 0; columnIdx < targetTable.getColumnCount(); columnIdx++) {
			...
			if (sourceColumn != null) {
				ColumnDefinitionChange change = compareColumns(
						intermediateTable, sourceColumn, targetTable,
						targetColumn);
	...
	/**
	 * Creates change objects for primary key differences (primary key
	 * added/removed/changed), and applies them to the given intermediate model.
	 * 
	 * @param sourceModel
	 *            The source model
	 * @param sourceTable
	...
	protected List checkForPrimaryKeyChanges(Database sourceModel,
		...
		if ((curPK.length == 0) && (targetPK.length > 0)) {
			...
			AddPrimaryKeyChange change = new AddPrimaryKeyChange(
					intermediateTable.getName(), getIntermediateColumnNamesFor(
							targetPK, intermediateTable));

			changes.add(change);
			change.apply(intermediateModel, _caseSensitive);
		} else if ((targetPK.length == 0) && (curPK.length > 0)) {
			if (_log.isInfoEnabled()) {
				_log.info("The primary key needs to be removed from the table "
						+ intermediateTable.getName());
			}

			RemovePrimaryKeyChange change = new RemovePrimaryKeyChange(
					intermediateTable.getName());
			...
			if ((curPK.length != targetPK.length)
					|| (!_canDropPrimaryKeyColumns && sourcePK.length > targetPK.length)) {
				changePK = true;
			} else if ((curPK.length > 0) && (targetPK.length > 0)) {
				for (int pkColumnIdx = 0; (pkColumnIdx < curPK.length)
						&& !changePK; pkColumnIdx++) {
					if (!StringUtilsExt.equals(curPK[pkColumnIdx].getName(),
							targetPK[pkColumnIdx].getName(), _caseSensitive)) {
						changePK = true;
					}
				}
			}
	...
	/**
	 * Compares the two columns and returns the change necessary to create the
	 * second column from the first one if they differe.
	 * 
	 * @param sourceTable
	...
	protected ColumnDefinitionChange compareColumns(Table sourceTable,
			Column sourceColumn, Table targetTable, Column targetColumn) {
		if (ColumnDefinitionChange.isChanged(getPlatformInfo(), sourceColumn,
				targetColumn)) {
			Column newColumnDef = _cloneHelper.clone(sourceColumn, true);
			int targetTypeCode = _platformInfo.getTargetJdbcType(targetColumn
					.getTypeCode());
			boolean sizeMatters = _platformInfo.hasSize(targetTypeCode);
			boolean scaleMatters = _platformInfo
					.hasPrecisionAndScale(targetTypeCode);

			newColumnDef.setTypeCode(targetColumn.getTypeCode());
			newColumnDef.setSize(sizeMatters || scaleMatters ? targetColumn
					.getSize() : null);
			...
			return new ColumnDefinitionChange(sourceTable.getName(),
					sourceColumn.getName(), newColumnDef);
		} else {
			return null;
		}
	}
