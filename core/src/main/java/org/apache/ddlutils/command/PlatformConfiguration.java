...
/**
 * Encloses the platform configuration for the command. One can think of this as
 * a command configuration. For example, parameters and switches, should go
 * here.
 * 
 * <b> Can be renamed to CommandConfiguration</b>
 * 
...
public class PlatformConfiguration {
	 ...
	 * @return The platform
	 */
	public Platform getPlatform() throws CommandException {
		if (_platform == null) {
			...
			try {
				_platform = PlatformFactory
						.createNewPlatformInstance(_databaseType);
			} catch (Exception ex) {
				throw new CommandException("Database type " + _databaseType
						+ " is not supported.", ex);
			}
			if (_platform == null) {
				throw new CommandException("Database type " + _databaseType
			...
			_platform
					.setDelimitedIdentifierModeOn(isUseDelimitedSqlIdentifiers());
