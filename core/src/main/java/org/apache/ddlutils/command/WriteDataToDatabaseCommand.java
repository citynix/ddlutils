...
public class WriteDataToDatabaseCommand extends ConvertingDatabaseCommand
{
    ...
    /** The input files. */
    private List<File> _fileSets = new LinkedList<File>();
     ...
     */
    public void addConfiguredFileset(List<File> fileset)
    {
        _fileSets.addAll(fileset);
    }
    ...
    public void execute() throws CommandException
    {
        ...
        {
            throw new CommandException("Please use either the datafile attribute or the sub fileset element, but not both");
        }
        ...
        try
        {
            ...
            {
                readSingleDataFile( dataReader, _singleDataFile);
            }
            else
            {
                for (File dataFile:  this._fileSets)
                {
...
//                    {
                        readSingleDataFile( dataReader, dataFile );
//                    }
                }
     ...
     */
    private void readSingleDataFile(DataReader reader, File dataFile) throws CommandException
    {
        if (!dataFile.exists())
        {
