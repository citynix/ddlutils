...
public class ColumnXmlWriter extends ModelXmlWriter {
	...
	public ColumnXmlWriter(Column column, String value) {
		/*
		 * - attribute "column name"="column value" in the parent's (table)
		 * element iff the column name is a valid attribute name and is not
		 * "table-name" and not "column", and the value is a valid attribute
		 * value not longer than 255 characters - otherwise, writes a
		 * sub-element <column> with an attribute column-name that contains the
		 * name of the column, and the body of that sub-element contains the
		 * column value, iff the column name is a valid attribute value not
		 * longer than 255 characters. If the column value contains illegal
		 * characters, then the column sub element will have a "base64"
		 * attribute with the value "true" and the value will be base64 encoded
		 * - otherwise writes a sub-element <column> with a sub-element
		 * <column-name> whose body is the name of the column, and another
		 * sub-element <column-value> whose body contains the column value. If
		 * either the column name or value contain illegal characters, then the
		 * corresponding sub element will have a "base64" attribute with the
		 * value "true" and its body will be base64 encoded.
		 */
		...
		if (XMLUtils.hasIllegalXMLCharacters(column.getName())) {
			...
			} else if (valueBase64Encoded
					|| (value.length() > XMLUtils.MAX_ATTRIBUTE_LENGTH)) {
	...
	/**
	 * Writes any sub elements necessary for the column. If no sub elements are
	 * required, then this method does nothing.
	 * 
	 * @param writer
	 *            The writer to write to
	 * @return <code>true</code> if something was written
