...
/**
 * Base interface for different strategies to write the XML for a data bean for
 * a specific table.
 */
public class TableXmlWriter extends ModelXmlWriter {
		...
		if (XMLUtils.hasIllegalXMLCharacters(table.getName())) {
			...
			if (tableName.length() > XMLUtils.MAX_NAME_LENGTH) {
				formattingMethod = AS_SUB_TAG;
					...
					|| !XMLUtils.isWellFormedXMLName(tableName)) {
	 ...
	 * 
	 * @param columnXmlWriters
	 *            A list of column xml writers for writing out the bean's values
	 *            to XML
	 * @param writer
	...
	public void write(List columnXmlWriters, DataWriter writer) {
		...
		for (Iterator it = columnXmlWriters.iterator(); it.hasNext();) {
			hasSubTags = ((ColumnXmlWriter) it.next()).writeSubElement(writer)
					|| hasSubTags;
		}
		if (hasSubTags) {
			writer.printlnIfPrettyPrinting();
