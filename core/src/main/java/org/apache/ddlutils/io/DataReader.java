...
public class DataReader {
		...
		try {
			...
			if (input != null) {
				try {
					...
					_log.warn(
							"Error while trying to close the input stream for "
									+ file, ex);
				}
			}
		}
	 ...
	 * 
	 * @param reader
	 *            The reader that returns the data XML
	 */
	 ...
	 * 
	 * @param input
	 *            The input stream that returns the data XML
	 */
	 ...
	 *            The reader
	 */
	private void readBean(XMLStreamReader xmlReader) throws XMLStreamException,
		...
		for (int idx = 0; idx < xmlReader.getAttributeCount(); idx++) {
			...
			attributes.put(isCaseSensitive()
					? attrQName.getLocalPart()
					: attrQName.getLocalPart().toLowerCase(), xmlReader
		...
		if (table == null) {
			_log.warn("Data XML contains an element "
					+ elemQName
					+ " at location "
					+ location
			...
			for (int idx = 0; idx < table.getColumnCount(); idx++) {
				...
				String value = (String) attributes.get(isCaseSensitive()
						? column.getName()
	...
	/**
	 * Reads all relevant sub elements that match the columns specified by the
	 * given table object from the xml reader into the given bean.
	 * 
	 * @param xmlReader
	 *            The reader
	 * @param data
	...
	/**
	 * Reads the next column sub element that matches a column specified by the
	 * given table object from the xml reader into the given bean.
	 * 
	 * @param xmlReader
	 *            The reader
	...
	/**
	 * Converts the column value read from the XML stream to an object and sets
	 * it at the given bean.
	 * 
	 * @param bean
	 *            The bean
	 * @param table
	 ...
	 */
	private void setColumnValue(DynaBean bean, Table table, Column column,
			String value) throws DdlUtilsXMLException {
		SqlTypeConverter converter = _converterConf.getRegisteredConverter(
				table, column);
		Object propValue = (converter != null ? converter.convertFromString(
				value, column.getTypeCode()) : value);

		try {
			PropertyUtils.setProperty(bean, column.getName(), propValue);
		} catch (NoSuchMethodException ex) {
			throw new DdlUtilsXMLException("Undefined column "
					+ column.getName());
		} catch (IllegalAccessException ex) {
			throw new DdlUtilsXMLException(
					"Could not set bean property for column "
							+ column.getName(), ex);
