...
public class SybaseBuilder extends SqlBuilder {
	...
	protected void copyData(Table sourceTable, Table targetTable)
			throws IOException {
		// We need to turn on identity override except when the identity column
		...
		if (targetAutoIncrCols.length > 0) {
			...
			if (sourceTable.findColumn(targetAutoIncrCols[0].getName(),
	 ...
	 */
	public void changeColumn(Table table, Column column, Column newColumn)
			throws IOException {
