...
public class MSSqlBuilder extends SqlBuilder {
	...
	protected String getValueAsString(Column column, Object value) {
		...
		switch (column.getTypeCode()) {
			...
			case Types.DATE :
				...
				result.append(value instanceof String
						? (String) value
	...
	protected String getNativeDefaultValue(Column column) {
		...
		if ((column.getTypeCode() == Types.BIT)
				|| (column.getTypeCode() == Types.BOOLEAN)) {
			return getDefaultValueHelper().convert(column.getDefaultValue(),
					column.getTypeCode(), Types.SMALLINT);
		} else {
	...
	/**
	 * If quotation mode is on, then this writes the statement that turns on the
	 * ability to write delimited identifiers.
	 */
	...
	protected void copyData(Table sourceTable, Table targetTable)
		...
		// - there is a column in both tables that is auto increment only in the
		// target table, or
		...
		// Sql Server allows only one identity column, so let's take a shortcut
		// here
	...
	/**
	 * Writes the SQL to recreate a column, e.g. using a different type or
	 * similar.
	 * 
	 * @param table
	 *            The table
	 ...
	 */
	public void recreateColumn(Table table, Column curColumn, Column newColumn)
			throws IOException {
		...
		if (shallHaveDefault) {
			...
			printIdentifier(getConstraintName("DF", table, curColumn.getName(),
					null));
	...
	/**
	 * Writes the SQL to drop a constraint, e.g. a primary key or default value
	 * constraint.
	 * 
	 * @param table
	 *            The table that the constraint is on
	 * @param column
	 *            The column that the constraint is on; <code>null</code> for
	 *            table-level constraints
	 * @param typeIdentifier
	 *            The constraint type identifier as is specified for the
	 *            <code>sysobjects</code> system table
	 */
	protected void writeDropConstraintStatement(Table table, Column column,
			String typeIdentifier) throws IOException {
		String tableName = getTableName(table);
	...
	protected void writeCastExpression(Column sourceColumn, Column targetColumn)
			throws IOException {
		boolean sizeChanged = ColumnDefinitionChange.isSizeChanged(
				getPlatformInfo(), sourceColumn, targetColumn);
		boolean typeChanged = ColumnDefinitionChange.isTypeChanged(
				getPlatformInfo(), sourceColumn, targetColumn);

		if (sizeChanged || typeChanged) {
			if (TypeMap.isTextType(targetColumn.getTypeCode())
					&& sizeChanged
					&& (targetColumn.getSize() != null)
					&& (sourceColumn.getSizeAsInt() > targetColumn
							.getSizeAsInt())) {
