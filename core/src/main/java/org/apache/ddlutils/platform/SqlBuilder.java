...
public abstract class SqlBuilder {
	 ...
	 * usually suffices to redefine the
	 ...
	 */
	public void createPrimaryKey(Table table, Column[] primaryKeyColumns)
			throws IOException {
	...
	public void createIndex(Table table, Index index) throws IOException {
		if (!getPlatformInfo().isIndicesSupported()) {
			throw new DdlUtilsException(
					"This platform does not support indexes");
	 ...
	 *            The table
	 */
	public void createForeignKeys(Database database, Table table)
			throws IOException {
	 ...
	 *            The foreign key
	 */
	public void createForeignKey(Database database, Table table,
			ForeignKey foreignKey) throws IOException {
		if (getPlatformInfo().isForeignKeysEmbedded()) {
			throw new DdlUtilsException(
					"This platform does not supported the external creation of foreign keys");
			...
			print(") REFERENCES ");
			printIdentifier(getTableName(database.findTable(foreignKey
					.getForeignTableName())));
			print(" (");
	 ...
	 *            The new column
	 */
	public void addColumn(Database model, Table table, Column newColumn)
			throws IOException {
		print("ALTER TABLE ");
		printlnIdentifier(getTableName(table));
		printIndent();
	 ...
	 *            The foreign key
	 */
	public void dropForeignKey(Table table, ForeignKey foreignKey)
			throws IOException {
		writeTableAlterStmt(table);
		...
		if (genPlaceholders) {
			...
			for (int idx = 0; idx < table.getColumnCount(); idx++) {
				...
				if (columnValues.containsKey(column.getName())) {
					...
					buffer.append(getValueAsString(column,
							columnValues.get(column.getName())));
					addComma = true;
				}
			}
		}
	 ...
	 * @return The update sql
	 */
	public String getUpdateSql(Table table, Map oldColumnValues,
			Map newColumnValues, boolean genPlaceholders) {
		StringBuffer buffer = new StringBuffer("UPDATE ");
		boolean addSep = false;
		...
		for (int idx = 0; idx < table.getColumnCount(); idx++) {
			...
			if (newColumnValues.containsKey(column.getName())) {
				...
				if (genPlaceholders) {
					...
					buffer.append(getValueAsString(column,
							newColumnValues.get(column.getName())));
				}
				addSep = true;
			}
		}
		...
		for (int idx = 0; idx < table.getColumnCount(); idx++) {
			...
			if (oldColumnValues.containsKey(column.getName())) {
				...
				if (genPlaceholders) {
					...
					buffer.append(getValueAsString(column,
							oldColumnValues.get(column.getName())));
				}
				addSep = true;
			}
		}
	 ...
	 * Depending on the value of <code>genPlaceholders</code>, the generated SQL
	 * will contain prepared statement place holders or concrete values. Only
	 * those primary key columns wil be used that are present in the given map.
	 * If the map is null or completely empty, then the SQL will not have a
	 * WHERE clause. The SQL will contain the columns in the order defined in
	 * the table.
	 * 
	 * @param table
	 *            The table
	...
	protected String getSqlType(Column column, String nativeType) {
		...
		sqlType.append(sizePos >= 0
				? nativeType.substring(0, sizePos)
				: nativeType);
	...
	protected String getNativeType(Column column) {
		String nativeType = (String) getPlatformInfo().getNativeType(
				column.getTypeCode());
	...
	/**
	 * Returns the size specification for the given column. If the column is of
	 * a type that has size or precision and scale, and no size is defined for
	 * the column itself, then the default size or precision/scale for that type
	 * and platform is used instead.
	 * 
	 * @param column
	 *            The column
	...
	protected void writeColumnDefaultValueStmt(Table table, Column column)
		...
		if (parsedDefault != null) {
			if (!getPlatformInfo().isDefaultValuesForLongTypesSupported()
					&& ((column.getTypeCode() == Types.LONGVARBINARY) || (column
	...
	public String getForeignKeyName(Table table, ForeignKey fk) {
		...
		if (needsName) {
			_log.warn("Encountered a foreign key in table "
					+ table.getName()
					+ " that has no name. "
	 ...
	 *            The foreignkey
	 */
	protected void writeForeignKeyOnDeleteAction(Table table,
			ForeignKey foreignKey) throws IOException {
		...
		if (!getPlatformInfo().isActionSupportedForOnDelete(action)) {
			if (getPlatform().isDefaultOnDeleteActionUsedIfUnsupported()) {
				_log.info("The platform does not support the " + action
						+ " action for onDelete; using "
						+ getPlatformInfo().getDefaultOnDeleteAction()
						+ " instead");
				action = getPlatformInfo().getDefaultOnDeleteAction();
			} else {
				throw new ModelException(
						"The platform does not support the action '" + action
								+ "' for onDelete in foreign key in table "
								+ table.getName());
			}
		}
	 ...
	 *            The foreignkey
	 */
	protected void writeForeignKeyOnUpdateAction(Table table,
			ForeignKey foreignKey) throws IOException {
		...
		if (!getPlatformInfo().isActionSupportedForOnUpdate(action)) {
			if (getPlatform().isDefaultOnUpdateActionUsedIfUnsupported()) {
				...
				throw new ModelException(
						"The platform does not support the action '" + action
								+ "' for onUpdate in foreign key in table "
								+ table.getName());
			}
		}
