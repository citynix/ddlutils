...
public class MckoiModelReader extends JdbcModelReader {
	...
	protected Table readTable(DatabaseMetaDataWrapper metaData, Map values)
			throws SQLException {
		// Mckoi does not currently return unique indices in the metadata so we
		// have to query
		// internal tables to get this info
		final String query = "SELECT uniqueColumns.column, uniqueColumns.seq_no, uniqueInfo.name"
		...
		if (table != null) {
			...
			try {
				stmt = getConnection().prepareStatement(
						table.getSchema() == null ? query : queryWithSchema);
				...
				while (resultSet.next()) {
					...
					indexValues.put("ORDINAL_POSITION",
							new Short(resultSet.getShort(2)));
