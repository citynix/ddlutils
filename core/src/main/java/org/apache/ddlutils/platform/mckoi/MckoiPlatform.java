	...
	public void createDatabase(String jdbcDriverClassName,
		...
		if (JDBC_DRIVER.equals(jdbcDriverClassName)) {
			...
			throw new UnsupportedOperationException(
					"Unable to create a McKoi database via the driver "
							+ jdbcDriverClassName);
		}
	}
	...
	protected TableDefinitionChangesPredicate getTableDefinitionChangesPredicate() {
		return new DefaultTableDefinitionChangesPredicate() {
			public boolean areSupported(Table intermediateTable, List changes) {
				// McKoi has this nice ALTER CREATE TABLE statement which saves
				// us a lot of work
				// Thus, we reject all table level changes and instead redefine
				// the handling of the
				// RecreateTableChange
				return false;
			}
	...
	public void processChange(Database currentModel, CreationParameters params,
		...
		for (Iterator it = change.getOriginalChanges().iterator(); it.hasNext();) {
			...
			if (tableChange instanceof ColumnDefinitionChange) {
				...
				Column origColumn = changedTable.findColumn(
						colChange.getChangedColumn(),
						isDelimitedIdentifierModeOn());
				Column newColumn = colChange.getNewColumn();

				if (!origColumn.isAutoIncrement()
						&& newColumn.isAutoIncrement()) {
					sqlBuilder.createAutoIncrementSequence(changedTable,
							origColumn);
				}
		...
		Map parameters = (params == null ? null : params
				.getParametersFor(changedTable));

		sqlBuilder.writeRecreateTableStmt(currentModel,
				change.getTargetTable(), parameters);

		// we have to defer removal of the sequences until they are no longer
		// used
		for (Iterator it = change.getOriginalChanges().iterator(); it.hasNext();) {
			TableChange tableChange = (TableChange) it.next();
			...
			if (tableChange instanceof ColumnDefinitionChange) {
				...
				if (origColumn.isAutoIncrement()
						&& !newColumn.isAutoIncrement()) {
					sqlBuilder.dropAutoIncrementSequence(changedTable,
							origColumn);
				}
