...
public class JdbcModelReader {
	...
	public String[] getDefaultTableTypes() {
		if (_defaultTableTypes == null) {
			...
			System.arraycopy(_defaultTableTypes, 0, result, 0,
					_defaultTableTypes.length);
			return result;
		}
	}
	...
	protected Collection readTables(String catalog, String schemaPattern,
		...
		try {
			...
			metaData.setCatalog(catalog == null
	...
	protected void removeSystemIndices(DatabaseMetaDataWrapper metaData,
		...
		for (int fkIdx = 0; fkIdx < table.getForeignKeyCount(); fkIdx++) {
			removeInternalForeignKeyIndex(metaData, table,
					table.getForeignKey(fkIdx));
		}
	}
	...
	protected void readForeignKey(DatabaseMetaDataWrapper metaData, Map values,
		...
		if (fk == null) {
			...
			CascadeActionEnum onUpdateAction = convertAction((Short) values
					.get("UPDATE_RULE"));
			CascadeActionEnum onDeleteAction = convertAction((Short) values
					.get("DELETE_RULE"));
	...
	protected Collection readIndices(DatabaseMetaDataWrapper metaData,
		...
		try {
			indexData = metaData.getIndices(
					metaData.escapeForSearch(tableName), false, false);
