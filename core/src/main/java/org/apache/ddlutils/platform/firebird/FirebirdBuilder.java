...
public class FirebirdBuilder extends SqlBuilder {
	 ...
	 *            The new column
	 * @param prevColumn
	 *            The column after which the new column shall be added;
	 *            <code>null</code> if the new column is to be inserted at the
	 *            beginning
	 */
	public void insertColumn(Database model, Table table, Column newColumn,
			Column prevColumn) throws IOException {
	...
	protected void writeCastExpression(Column sourceColumn, Column targetColumn)
		...
		if (sizeChanged || typeChanged) {
			boolean needSubstr = TypeMap.isTextType(targetColumn.getTypeCode())
					&& sizeChanged
			...
			// we're not using CAST but instead string construction which does
			// not require us to know the size
			print("(");
