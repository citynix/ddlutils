...
public class SapDbPlatform extends PlatformImplBase {
	...
	public SapDbPlatform() {
		...
		info.addNativeTypeMapping(Types.DATALINK, "LONG BYTE",
				Types.LONGVARBINARY);
	...
	protected TableDefinitionChangesPredicate getTableDefinitionChangesPredicate() {
		return new DefaultTableDefinitionChangesPredicate() {
			protected boolean isSupported(Table intermediateTable,
					TableChange change) {
				if ((change instanceof RemoveColumnChange)
					...
					// SapDB can only add not insert columns, and required
					// columns have to have
					// a default value or be IDENTITY
					return (addColumnChange.getNextColumn() == null)
							&& (!addColumnChange.getNewColumn().isRequired() || !StringUtilsExt
									.isEmpty(addColumnChange.getNewColumn()
											.getDefaultValue()));
				} else if (change instanceof ColumnDefinitionChange) {
					ColumnDefinitionChange colChange = (ColumnDefinitionChange) change;

					// SapDB has a ALTER TABLE MODIFY COLUMN but it is limited
					// regarding the type conversions
					// it can perform, so we don't use it here but rather
					// rebuild the table
					Column curColumn = intermediateTable.findColumn(
							colChange.getChangedColumn(),
							isDelimitedIdentifierModeOn());
					Column newColumn = colChange.getNewColumn();

					// we can however handle the change if only the default
					// value or the required status was changed
					return ((curColumn.getTypeCode() == newColumn.getTypeCode())
							&& !ColumnDefinitionChange.isSizeChanged(
									getPlatformInfo(), curColumn, newColumn) && (curColumn
								.isAutoIncrement() == newColumn
							.isAutoIncrement()));
				} else {
					return false;
				}
	...
	public void processChange(Database currentModel, CreationParameters params,
		...
		Column removedColumn = changedTable.findColumn(
				change.getChangedColumn(), isDelimitedIdentifierModeOn());

		((SapDbBuilder) getSqlBuilder())
				.dropColumn(changedTable, removedColumn);
	...
	public void processChange(Database currentModel, CreationParameters params,
		...
		for (int colIdx = 0; colIdx < newPKColumnNames.length; colIdx++) {
			newPKColumns[colIdx] = changedTable.findColumn(
					newPKColumnNames[colIdx], isDelimitedIdentifierModeOn());
		}
	...
	public void processChange(Database currentModel, CreationParameters params,
		...
		Table changedTable = findChangedTable(currentModel, change);
		Column changedColumn = changedTable.findColumn(
				change.getChangedColumn(), isDelimitedIdentifierModeOn());

		if (!StringUtilsExt.equals(changedColumn.getDefaultValue(), change
				.getNewColumn().getDefaultValue())) {
			((SapDbBuilder) getSqlBuilder()).changeColumnDefaultValue(
					changedTable, changedColumn, change.getNewColumn()
							.getDefaultValue());
		}
		if (changedColumn.isRequired() != change.getNewColumn().isRequired()) {
			((SapDbBuilder) getSqlBuilder()).changeColumnRequiredStatus(
					changedTable, changedColumn, change.getNewColumn()
							.isRequired());
		}
		change.apply(currentModel, isDelimitedIdentifierModeOn());
