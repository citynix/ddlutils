...
public class CloneHelper {
	/**
	 * Returns a deep clone of the given model object, including all tables,
	...
	public Database clone(Database source) {
		...
		for (int tableIdx = 0; tableIdx < source.getTableCount(); tableIdx++) {
			...
			for (int fkIdx = 0; fkIdx < sourceTable.getForeignKeyCount(); fkIdx++) {
				...
				clonedTable.addForeignKey(clone(sourceFk, clonedTable, result,
						true));
			}
		}
		return result;
	 ...
	 *            The source table
	 * @param cloneIndexes
	 *            Whether to clone indexes; if <code>false</code> then the clone
	 *            will not have any indexes
	 * @param cloneForeignKeys
	 *            Whether to clone foreign kets; if <code>false</code> then the
	 *            clone will not have any foreign keys
	 * @param targetModel
	 *            The target model, can be <code>null</code> if
	 *            <code>cloneForeignKeys=false</code>
	 * @param caseSensitive
	 *            Whether comparison is case sensitive (for cloning foreign
	 *            keys)
	 * @return The clone
	 */
	public Table clone(Table source, boolean cloneIndexes,
			boolean cloneForeignKeys, Database targetModel,
		...
		if (cloneForeignKeys) {
			for (int fkIdx = 0; fkIdx < source.getForeignKeyCount(); fkIdx++) {
				result.addForeignKey(clone(source.getForeignKey(fkIdx), result,
						targetModel, caseSensitive));
			}
		}
	 ...
	 * @param source
	 *            The source column
	 * @param clonePrimaryKeyStatus
	 *            Whether to clone the column's primary key status; if
	 *            <code>false</code> then the clone will not be a primary key
	 *            column
	 * @return The clone
	 */
	public Column clone(Column source, boolean clonePrimaryKeyStatus) {
		...
		result.setPrimaryKey(clonePrimaryKeyStatus
				? source.isPrimaryKey()
				: false);
	 ...
	 *            The source index
	 * @param targetTable
	 *            The table whose columns shall be used by the clone
	 * @param caseSensitive
	 *            Whether comparison is case sensitive (for finding the columns
	 *            in the target table)
	 * @return The clone
	 */
	public Index clone(Index source, Table targetTable, boolean caseSensitive) {
		Index result = (source.isUnique()
	 ...
	 * @param targetTable
	 *            The table containing the column to be used by the clone
	 * @param caseSensitive
	 *            Whether comparison is case sensitive (for finding the columns
	 *            in the target table)
	 * @return The clone
	 */
	public IndexColumn clone(IndexColumn source, Table targetTable,
			boolean caseSensitive) {
	 ...
	 * @param owningTable
	 *            The table owning the source foreign key
	 * @param targetModel
	 *            The target model containing the tables that the clone shall
	 *            link
	 * @param caseSensitive
	 *            Whether comparison is case sensitive (for finding the columns
	 *            in the target model)
	 * @return The clone
	 */
	public ForeignKey clone(ForeignKey source, Table owningTable,
			Database targetModel, boolean caseSensitive) {
		ForeignKey result = new ForeignKey();
		Table foreignTable = targetModel.findTable(
				source.getForeignTableName(), caseSensitive);
		...
		for (int refIdx = 0; refIdx < source.getReferenceCount(); refIdx++) {
			...
			result.addReference(clone(ref, owningTable, foreignTable,
					caseSensitive));
		}
	 ...
	 * @param localTable
	 *            The table containing the local column to be used by the
	 *            reference
	 * @param foreignTable
	 *            The table containing the foreign column to be used by the
	 *            reference
	 * @param caseSensitive
	 *            Whether comparison is case sensitive (for finding the columns
	 *            in the tables)
	 * @return The clone
	 */
	public Reference clone(Reference source, Table localTable,
			Table foreignTable, boolean caseSensitive) {
		Reference result = new Reference();

		result.setLocalColumn(localTable.findColumn(
				source.getLocalColumnName(), caseSensitive));
		result.setForeignColumn(foreignTable.findColumn(
				source.getForeignColumnName(), caseSensitive));
		return result;
	}
}
