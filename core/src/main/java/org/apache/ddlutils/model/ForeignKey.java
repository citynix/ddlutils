...
public class ForeignKey implements Serializable {
	...
	/**
	 * Returns the action for this foreignkey for when the referenced row is
	 * deleted.
	 * 
	 * @return The action
	 */
	...
	/**
	 * Sets the action for this foreignkey for when the referenced row is
	 * deleted.
	 * 
	 * @param onDelete
	 *            The action
	 * @throws NullPointerException
	 *             If <code>onDelete</code> is null
	 */
	public void setOnDelete(CascadeActionEnum onDelete)
			throws NullPointerException {
		if (onDelete == null) {
	 ...
	 *            The action
	 * @throws NullPointerException
	 *             If <code>onUdate</code> is null
	 */
	public void setOnUpdate(CascadeActionEnum onUpdate)
			throws NullPointerException {
		if (onUpdate == null) {
	 ...
	 * @param caseSensitive
	 *            Whether case matters when checking for the column's name
	 * @return <code>true</code> if a reference uses the column as a local
	 *         column
	 */
	public boolean hasLocalColumn(String columnName, boolean caseSensitive) {
		for (int idx = 0; idx < getReferenceCount(); idx++) {
			if (StringUtilsExt.equals(columnName, getReference(idx)
					.getLocalColumnName(), caseSensitive)) {
				return true;
			}
		}
		return false;
	...
	/**
	 * Determines whether this foreign key uses the given column as a foreign
	 * column in a reference. This method assumes that the caller already
	 * checked whether this foreign key references the tale owning the indicate
	 * column.
	 * 
	 * @param columnName
	 *            The name of the column to check
	...
	public boolean hasForeignColumn(String columnName, boolean caseSensitive) {
		for (int idx = 0; idx < getReferenceCount(); idx++) {
			if (StringUtilsExt.equals(columnName, getReference(idx)
					.getForeignColumnName(), caseSensitive)) {
				return true;
			}
		}
		return false;
	 ...
	 * index. Note that this is a hint for the platform and has no relevancy to
	 * the model itself.
	 * 
	 ...
	 * index. Note that this is a hint set by the model reader and has no
	 * relevancy to the model itself.
	 * 
	...
	public int hashCode() {
		return new HashCodeBuilder(17, 37).append(_name)
				.append(_foreignTableName).append(_references).toHashCode();
	}
