...
public class CascadeActionEnum extends ValuedEnum {
	...
	/**
	 * The enum value for a cascade action which directs the database to apply
	 * the change to the referenced table also to this table. E.g. if the
	 * referenced row is deleted, then the local one will also be deleted when
	 * this value is used for the onDelete action.
	 */
	public static final CascadeActionEnum CASCADE = new CascadeActionEnum(
			"cascade", VALUE_CASCADE);
	/**
	 * The enum value for a cascade action which directs the database to set the
	 * local columns referenced by the foreign key to null when the referenced
	 * row changes/is deleted.
	 */
	public static final CascadeActionEnum SET_NULL = new CascadeActionEnum(
			"setnull", VALUE_SET_NULL);
	/**
	 * The enum value for a cascade action which directs the database to set the
	 * local columns referenced by the foreign key to the default value when the
	 * referenced row changes/is deleted.
	 */
	public static final CascadeActionEnum SET_DEFAULT = new CascadeActionEnum(
			"setdefault", VALUE_SET_DEFAULT);
	/**
	 * The enum value for a cascade action which directs the database to
	 * restrict the change changes to the referenced column. The interpretation
	 * of this is database-dependent, but it is usually the same as
	 * {@link #NONE}.
	 */
	public static final CascadeActionEnum RESTRICT = new CascadeActionEnum(
			"restrict", VALUE_RESTRICT);
	/**
	 * The enum value for the cascade action that directs the database to not
	 * change the local column when the value of the referenced column changes,
	 * only check the foreign key constraint.
	 */
	public static final CascadeActionEnum NONE = new CascadeActionEnum("none",
			VALUE_NONE);
	...
	public static CascadeActionEnum getEnum(String defaultTextRep) {
		return (CascadeActionEnum) getEnum(CascadeActionEnum.class,
				defaultTextRep);
	}
