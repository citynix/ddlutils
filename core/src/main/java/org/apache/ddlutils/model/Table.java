...
public class Table implements Serializable {
	...
	/**
	 * Removes all columns of this table. Note that this does not change indexes
	 * or foreign keys, so it might leave the table object in an illegal state.
	 */
	...
	/**
	 * Finds the index with the specified name, using case sensitive or
	 * insensitive matching depending on the <code>caseSensitive</code>
	 * parameter. Note that this method is not called getIndex to avoid
	 * introspection problems.
	 * 
	 * @param name
	 *            The name of the index
	...
	public Index findIndex(String name, boolean caseSensitive) {
		if (name == null) {
			throw new NullPointerException(
					"The index name to search for cannot be null");
		}
	...
	/**
	 * Finds the foreign key with the specified name, using case insensitive
	 * matching. Note that this method is not called getForeignKey to avoid
	 * introspection problems.
	 * 
	 * @param name
	...
	/**
	 * Finds the foreign key with the specified name, using case sensitive or
	 * insensitive matching depending on the <code>caseSensitive</code>
	 * parameter. Note that this method is not called getForeignKey to avoid
	 * introspection problems.
	 * 
	 * @param name
	 *            The name of the foreign key
	...
	public ForeignKey findForeignKey(String name, boolean caseSensitive) {
		if (name == null) {
			throw new NullPointerException(
					"The foreign key name to search for cannot be null");
		}
		for (int idx = 0; idx < getForeignKeyCount(); idx++) {
			...
			if ((caseSensitive && name.equals(foreignKey.getName()))
					|| (!caseSensitive && name.equalsIgnoreCase(foreignKey
							.getName()))) {
				return foreignKey;
			}
		}
	...
	/**
	 * Returns the auto increment columns in this table. If none are found, then
	...
	public Column[] getRequiredColumns() {
		Collection requiredColumns = CollectionUtils.select(_columns,
				new Predicate() {
					public boolean evaluate(Object input) {
		...
		return (Column[]) requiredColumns.toArray(new Column[requiredColumns
				.size()]);
	}
		...
		if (!_foreignKeys.isEmpty()) {
			...
			Collections.sort(_foreignKeys, new Comparator() {
				public int compare(Object obj1, Object obj2) {
					...
					if (!caseSensitive) {
						fk1Name = (fk1Name != null
								? fk1Name.toLowerCase()
								: null);
						fk2Name = (fk2Name != null
								? fk2Name.toLowerCase()
								: null);
					}
		...
		if (obj instanceof Table) {
			...
			return new EqualsBuilder()
					...
					.append(new HashSet(_foreignKeys),
							new HashSet(other._foreignKeys))
