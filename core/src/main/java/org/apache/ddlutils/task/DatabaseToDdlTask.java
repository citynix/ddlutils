...
public class DatabaseToDdlTask extends DatabaseTaskBase {
	 ...
	 * sub tasks. For details and typical table types see <a href=
	...
	/**
	 * Sets the names of the tables that shall be read, as a comma-separated
	 * list. Escape a comma via '\,' if it is part of the table name. Please
	 * note that table names are not trimmed which means that whitespace
	 * characters should only be present in this string if they are actually
	 * part of the table name (i.e. in delimited identifer mode).
	 * 
	 * @param tableNameList
	 *            The comma-separated list of table names
	 * @ant.not-required If no table filter is specified, then all tables will
	 *                   be read unless <code>excludeTables</code> or
	 *                   <code>excludeTableFilter</code> is specifed
	 */
	public void setIncludeTables(String tableNameList) {
		_includeTableNames = new TaskHelper()
				.parseCommaSeparatedStringList(tableNameList);
	}

	/**
	 * Sets the regular expression matching the names of the tables that shall
	 * be read. For case insensitive matching, an uppercase name can be assumed.
	 * 
	...
	/**
	 * Sets the names of the tables that shall be ignored, as a comma-separated
	 * list. Escape a comma via '\,' if it is part of the table name. Please
	 * note that table names are not trimmed which means that whitespace
	 * characters should only be present in this string if they are actually
	 * part of the table name (i.e. in delimited identifer mode).
	 ...
	 *            The comma-separated list of table names
	 * @ant.not-required If no table filter is specified, then all tables will
	 *                   be read unless <code>includeTables</code> or
	 *                   <code>includeTableFilter</code> is specifed
	 */
	public void setExcludeTables(String tableNameList) {
		_excludeTableNames = new TaskHelper()
				.parseCommaSeparatedStringList(tableNameList);
	}

	/**
	 * Sets the regular expression matching the names of the tables that shall
	 * be ignored. For case insensitive matching, an uppercase name can be
	 * assumed.
	 * 
	 * @param tableNameRegExp
	 *            The regular expression; see {@link java.util.regex.Pattern}
	...
	protected Database readModel() {
		...
		try {
			...
			if ((_includeTableNames != null)
					|| (_includeTableNameRegExp != null)
					|| (_excludeTableNames != null)
					|| (_excludeTableNameRegExp != null)) {
				ModelHelper helper = new ModelHelper();

				if (_includeTableNames != null) {
					Table[] tables = model.findTables(_includeTableNames,
					...
					Table[] tables = model.findTables(_includeTableNameRegExp,
							getPlatformConfiguration()
									.isUseDelimitedSqlIdentifiers());
				...
				if (_excludeTableNames != null) {
					Table[] tables = model.findTables(_excludeTableNames,
							getPlatformConfiguration()
									.isUseDelimitedSqlIdentifiers());
					...
					Table[] tables = model.findTables(_excludeTableNameRegExp,
							getPlatformConfiguration()
									.isUseDelimitedSqlIdentifiers());
